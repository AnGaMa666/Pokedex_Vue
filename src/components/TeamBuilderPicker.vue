    ...
    >
      ...
      >
        <header class="picker-header">
          ...
          <button type="button" class="picker-close" :aria-label="closeLabel" @click="closePicker">
            ×
          </button>
        </header>
        ...
        <div v-if="loading" class="picker-status" role="status">
          {{ loadingLabel }}
        </div>
        <div v-else-if="filteredOptions.length === 0" class="picker-status">
          {{ emptyLabel }}
        </div>
        ...
        <ul v-else class="picker-list">
          <li v-for="option in visibleOptions" :key="option.value">
            ...
            >
              ...
              <span v-else class="picker-symbol" aria-hidden="true">
                {{ option.symbol || '◆' }}
              </span>
        ...
        <footer v-if="filteredOptions.length > visibleLimit" class="picker-footer">
          ...
          <button type="button" @click="visibleLimit += PAGE_SIZE">
            {{ moreLabel }}
          </button>
        </footer>
      </section>
...
.picker-search > span:not(.visually-hidden) {
  color: var(--legacy-muted);
  font-size: 1.4rem;
}
